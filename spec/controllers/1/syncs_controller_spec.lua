...
describe("SyncsController", function()
    ...
    describe("#account lifecycle", function()
        it("requires the rotated key for deletion and removes the preserved progress", function()
            local username, oldkey, newkey = "reader", "old-key", "new-key"
            local document = "document1"
            register(username, oldkey)
            update(username, oldkey, document, 0.32, "56", "reader device", "device1")
            local saved = get(username, oldkey, document).body

            assert.are.same(200, update_password(username, oldkey, newkey).status)
            assert.are.same(401, delete_user(username, oldkey).status)
            assert.are.same(saved, get(username, newkey, document).body)
            assert.are.same(200, delete_user(username, newkey).status)
            assert.are.same(401, authorize(username, newkey).status)
            assert.are.same(401, update_password(username, newkey, "another-key").status)

            assert.are.same(201, register(username, "fresh-key").status)
            assert.are.same({}, get(username, "fresh-key", document).body)
        end)
    end)
